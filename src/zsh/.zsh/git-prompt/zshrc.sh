...
git_super_status() {
    ...
    if [ -n "$__CURRENT_GIT_STATUS" ]; then
	  STATUS="%{${reset_color}%}$ZSH_THEME_GIT_PROMPT_PREFIX$ZSH_THEME_GIT_PROMPT_BRANCH$GIT_BRANCH%{${reset_color}%}"
	  ...
	  if [ "$GIT_STASHED" -ne "0" ]; then
		  STATUS="$STATUS$ZSH_THEME_GIT_PROMPT_STASHED<< STASH COUNT: $GIT_STASHED >>%{${reset_color}%}"
	  fi
...
ZSH_THEME_GIT_PROMPT_STAGED="%{\e[38;5;047m%}%}Δ"
ZSH_THEME_GIT_PROMPT_CONFLICTS="%{$fg_bold[red]%}x"
ZSH_THEME_GIT_PROMPT_CHANGED=$'%{\e[38;5;228m%}Δ'
ZSH_THEME_GIT_PROMPT_REMOTE=""
ZSH_THEME_GIT_PROMPT_UNTRACKED=$'%{\e[38;5;218m%}?'
